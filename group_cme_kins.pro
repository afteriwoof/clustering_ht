...
pro group_cme_kins, fls, ave_xs, ave_ys, slopes, pos_angs, fail, n_clusters=n_clusters,cme_count=cme_count, out_dir=out_dir, tog=tog ; count_kin_profs
...
for i=0,sz-1 do begin
        ...
        ;pos_angs[i] = strmid(filename,20,3)

        restore,fls[i]
        ; calculate the centre-of-gravity of the points
        ave_xs[i] = mean(definite_x)
        ave_ys[i] = mean(definite_y)
        ave_dt[i] = mean(anytim(datetime,/tai))
        definite_x = anytim(datetime,/tai)ave_xs[i] = ave_dt[i]
        ...
        if n_elements(uniq(definite_x)) gt 1 then begin
                ...
                slopes[i] = (model[n_elements(model)-1]-model[0])/(definite_x[n_elements(definite_x)-1]-definit
e_x[0])
                ;print, 'Slope: ', slopes[i]

                plot, definite_x, definite_y, psym=2, xr=[min(definite_x),max(definite_x)],/xs,yr=[0,2e4],/ys
...
if n_elements(n_clusters) eq 0 then n_clusters = 2
;clustering, adx,slp, n_clusters=5, /normalise
;print, ave_xs, slopes
;clustering, ave_xs, ave_ys, fail, cme_count=cme_count, out_dir=out_dir, n_clusters=n_clusters, /normalise;, /group_clusters
clustering, ave_xs, slopes, fail, cme_count=cme_count, out_dir=out_dir, n_clusters=n_clusters, /normalise, tog=tog;, /group_clusters
