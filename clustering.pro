...
pro clustering, ix, iy, fail, cme_count=cme_count, out_dir=out_dir, n_clusters=n_clusters, normalise=normalise, group_clusters=group_clusters, tog=tog


if n_elements(cme_count) eq 0 then cme_count=0

if n_elements(out_dir) eq 0 then begin
        out_dir='clustering_out_dir'
endif
...
; PLOT
plot, array[0,*], array[1,*], psym=3, /iso, /nodata, ytit='Slope of linear fit (normalised)', xtit='Average tim
e (normalised)'
set_line_color
for i=0,n_clusters-1 do begin
        plots,weights[0,i],weights[1,i],psym=2,color=0
        loc = where(result eq i,count)
        if count ne 0 then begin
                temp = array[*,loc]
                plots, temp, psym=([1,4,5,6,7,1,4,5,6,7])[i], color=([3,5,4,6,7,8,9])[i]
        endif
endfor
if keyword_set(tog) then begin
        set_plot, 'ps'
        loadct, 13
        device, /encapsul, bits=8, language=2, /portrait, /color, filename=out_dir+'/cluster_plot.eps', xs=15, 
ys=15
        !p.background=1
        !p.charsize=1
        !p.charthick=5
...
plot, array[0,*], array[1,*], psym=3, /iso, /nodata, ytit='Slope of linear fit (normalised)', xtit='Average time (normalised)', tit='Clusters'
set_line_color
for i=0,n_clusters-1 do begin
        plots,weights[0,i],weights[1,i],psym=2,color=0
        loc = where(result eq i,count)
...
if keyword_set(group_clusters) then begin
        ...
        for i=0,n_clusters-1 do begin
                ...
                plots,weights[0,i],weights[1,i],psym=2,color=6
                loc = where(result eq i,count)
